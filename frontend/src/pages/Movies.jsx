...
export default function Movies() {
  ...
  // Modified MovieRow component with premium UI & slide arrow headers
  const MovieRow = ({ title, icon, movies, isGatedDeleteView = null }) => {
    const rowRef = useRef();
    if (!movies || movies.length === 0) return null;

    return (
      <div className="relative mb-16 px-16 md:px-24 group">
        {title && (
          <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-6">
            <div className="flex items-center gap-3">
              {icon && <span className="text-red-500 text-3xl">{icon}</span>}
              <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-neutral-100">{title}</h2>
            </div>
            <button 
              onClick={() => {
                if (title.includes("Watchlist")) setActiveViewFilter("watchlist");
                else if (title.includes("History")) setActiveViewFilter("watched");
              }}
              className="flex items-center gap-1.5 text-neutral-400 font-semibold text-lg hover:text-white transition-colors duration-200"
            >
              View All <FaChevronRight className="text-sm mt-0.5" />
            </button>
          </div>
        )}
        ...
        <div className="relative group/row">
          ...
          <button 
            onClick={() => rowRef.current.scrollBy({ left: -700, behavior: "smooth" })} 
            className="absolute -left-6 top-1/2 z-30 -translate-y-1/2 w-12 h-12 rounded-full bg-black/80 border border-white/10 opacity-0 group-hover/row:opacity-100 transition-all hover:bg-neutral-900 flex items-center justify-center text-white shadow-xl"
          >
            <FaChevronLeft />
          </button>
          ...
          <button 
            onClick={() => rowRef.current.scrollBy({ left: 700, behavior: "smooth" })} 
            className="absolute -right-6 top-1/2 z-30 -translate-y-1/2 w-12 h-12 rounded-full bg-black/80 border border-white/10 opacity-0 group-hover/row:opacity-100 transition-all hover:bg-neutral-900 flex items-center justify-center text-white shadow-xl"
          >
            <FaChevronRight />
          </button>

          {/* Cards Tray */}
          <div ref={rowRef} className="flex gap-5 overflow-x-auto scrollbar-hide scroll-smooth py-2">
            {movies.map((movie) => (
              <div 
                key={movie.id} 
                onClick={() => movie.id !== "fallback" && navigate(`/movie/${movie.id}`)}
                className="group/card relative min-w-[280px] md:min-w-[310px] h-[440px] rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_10px_30px_rgba(0,0,0,0.8)] bg-zinc-950"
              >
                {/* Vertical Poster Style Mapping */}
                {movie.poster_path ? (
                  <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="w-full h-full object-cover transition-all duration-500 group-hover/card:scale-105" />
                ) : movie.backdrop_path ? (
                  <img src={`${IMG}${movie.backdrop_path}`} alt={movie.title} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-[#121218] flex items-center justify-center text-white/20 text-sm font-mono">NO POSTER</div>
                )}
                ...
                <div className="absolute top-4 left-4 right-4 flex items-center justify-between z-30">
                  <div className="px-3 py-1 rounded-full bg-black/60 backdrop-blur-md text-yellow-500 font-bold flex items-center gap-1.5 text-xs border border-white/10">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-neutral-800 transition-all duration-200"
                    title="More Info"
                  >
                ...
                <div className="absolute bottom-0 left-0 w-full p-5 z-20">
                  ...
                  <p className="text-white/50 text-xs">{movie.release_date?.split("-")[0] || "Unknown"}</p>
                </div>

                {/* Hover Quick Action Buttons Panel */}
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/card:opacity-100 transition-all duration-300 flex flex-col justify-end p-5 z-20">
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 py-2.5 rounded-xl bg-white text-black font-bold flex items-center justify-center gap-2 text-xs transition-transform active:scale-95"
                    >
                      <FaPlay className="text-[10px]" /> Trailer
                    </button>
                    
                    {isGatedDeleteView === "watchlist" ? (
                      <button onClick={(e) => removeFromWatchlist(e, movie)} className="w-11 h-9 rounded-xl bg-red-600/20 border border-red-500/30 flex items-center justify-center text-red-400 hover:bg-red-600 hover:text-white" title="Remove"><FaTrashAlt text-xs /></button>
                    ) : isGatedDeleteView === "watched" ? (
                      <button onClick={(e) => removeFromHistoryLog(e, movie)} className="w-11 h-9 rounded-xl bg-red-600/20 border border-red-500/30 flex items-center justify-center text-red-400 hover:bg-red-600 hover:text-white" title="Wipe Log"><FaTrashAlt text-xs /></button>
                    ) : (
                      <button 
                        onClick={(e) => { 
                          e.stopPropagation(); 
                        ...
                        className="w-11 h-9 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center text-white hover:bg-white/20"
                      >
  ...
  return (
    <div className="bg-[#030305] text-white min-h-screen overflow-x-hidden font-sans relative">
      ...
      {isSearchPageOpen ? (
        ...
        /* ── 🔍 MODE 1: SEARCH FEED INTERFACE ── */
        <div className="min-h-screen pl-16 md:pl-24 pt-10 pr-6 md:pr-12 flex flex-col space-y-10">
          ...
          <div className="w-full pt-4">
            {searchResults.length > 0 ? (
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-bold tracking-tight text-white mb-2 pl-16 md:pl-24 flex items-center gap-2">
        ...
        <>
            ...
            <div className="relative h-screen pl-16 md:pl-20">
              ...
              <div className="relative z-10 flex items-center h-full px-14 md:px-20">
                <div className="max-w-3xl">
                  ...
                  <h1 className="text-4xl md:text-6xl font-black leading-tight mb-6 tracking-tight">{heroMovie.title}</h1>
                  <p className="text-sm md:text-base text-white/70 leading-relaxed mb-10 line-clamp-3 font-medium">{heroMovie.overview}</p>
                  <div className="flex gap-4">
                    <button 
                      onClick={() => openTrailer(heroMovie)} 
                      className="px-6 py-3 rounded-xl bg-white text-black text-sm font-bold flex items-center gap-2 hover:scale-105 shadow-2xl transition-transform cursor-pointer"
                    >
                      <FaPlay className="text-[10px]" /> Watch Trailer
                    </button>
                    <button 
                      onClick={() => heroMovie.id !== "fallback" && navigate(`/movie/${heroMovie.id}`)} 
                      className="px-6 py-3 rounded-xl bg-white/10 border border-white/10 text-sm font-bold hover:bg-white/20 hover:scale-105 transition-transform cursor-pointer"
                    >
                      Watch Now
                    </button>
                  </div>
          ...
          <div className="relative z-20 -mt-24 pb-32 space-y-12">
      ...
      {selectedMovie && (
        <div className="fixed inset-0 z-[99999] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6">
          <div className="relative w-full max-w-5xl rounded-3xl overflow-hidden bg-[#0c0c12] border border-white/10 shadow-2xl animate-fade-in">
            <button onClick={() => setSelectedMovie(null)} className="absolute top-6 right-6 z-30 w-12 h-12 rounded-full bg-black/60 flex items-center justify-center text-xl hover:bg-red-500 text-white cursor-pointer"><FaTimes /></button>
            <div className="relative h-[550px]">
              ...
              <div className="absolute bottom-10 left-10 right-10 z-10">
                <h1 className="text-4xl font-black mb-3 tracking-tight">{selectedMovie.title}</h1>
                <div className="flex items-center gap-4 mb-3 text-xs font-semibold">
                  <span className="text-emerald-400 font-bold">{selectedMovie.vote_average?.toFixed(1) || "0.0"} Rating</span>
                  <span className="text-white/50">{selectedMovie.release_date || "Unknown date"}</span>
                </div>
                <p className="text-sm text-white/70 leading-relaxed mb-6 line-clamp-3">{selectedMovie.overview}</p>
                <div className="flex gap-3">
                  <button onClick={() => openTrailer(selectedMovie)} className="px-6 py-3 rounded-xl bg-white text-black font-bold text-sm flex items-center gap-2 hover:scale-105 transition-transform cursor-pointer"><FaPlay /> Watch Trailer</button>
                  <button 
                    onClick={() => {
                      setSelectedMovie(null);
                      navigate(`/movie/${selectedMovie.id}`);
                    }}
                    className="px-6 py-3 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 text-sm font-bold text-white hover:scale-105 transition-transform cursor-pointer"
                  >
                    Play Full Movie
                  </button>
                </div>
      ...
      {trailerKey && (
        <div className="fixed inset-0 z-[999999] bg-black/80 backdrop-blur-2xl flex items-center justify-center p-4">
          <button onClick={() => { setTrailerKey(""); setActiveTrailerMovie(null); }} className="absolute top-8 right-8 z-50 w-12 h-12 rounded-full bg-black/60 border border-white/10 text-xl flex items-center justify-center hover:bg-red-500 text-white cursor-pointer"><FaTimes /></button>
          <div className="w-[85vw] h-[75vh] rounded-2xl overflow-hidden border border-white/10 bg-black relative z-10 shadow-2xl">
