...
import { FaCloudUploadAlt, FaFilm, FaArrowLeft, FaSync, FaTv, FaSparkles } from "react-icons/fa";
...
export default function FindYourMovie() {
  ...
  // Match Result State (Mock response to simulate your future backend neural analysis model)
  ...
  const processFile = (file) => {
    ...
    // Strict guest token gate metrics
    ...
    setMatchedMovie(null); // Clear previous results
  };
  ...
  const triggerVisualAnalysis = async () => {
    ...
    // Simulated matched movie payload data
    const mockResult = {
      title: "Leo",
      year: "2023",
      rating: "6.9",
      ...
      posterPath: "https://image.tmdb.org/t/p/w500/yrp9R24m7f2Mv39MvHj3jN0jVRE.jpg" // Fallback fallback link placeholder
    };
    ...
    // Log the action to the user's active session logs in Firestore
    if (auth.currentUser) {
      try {
        ...
        console.error("Firestore log print failure:", err);
      }
    }
  };
    ...
    <div className="min-h-screen bg-[#04040a] text-white relative overflow-hidden font-sans antialiased">
      ...
      <div className="relative z-10 flex flex-col h-screen w-full">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-white/[0.06] backdrop-blur-2xl bg-black/20 shrink-0">
          ...
          <button
            onClick={() => navigate("/")}
            className="px-6 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 font-bold text-xs shadow-lg transition flex items-center gap-2"
          >
            <FaArrowLeft className="text-[9px]" /> Back to Home
          </button>
        </div>

        {/* Workspace Central Split Layout Grid */}
        <div className="flex-1 overflow-y-auto px-6 py-10 flex items-center justify-center bg-black/10">
          <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            {/* LEFT COLUMN: Media Input Console Dropzone Chassis */}
            <div className="w-full space-y-4">
              <div className="bg-[#0b0b14]/60 border border-white/[0.06] rounded-[32px] p-6 backdrop-blur-xl shadow-2xl">
                ...
                {!previewUrl ? (
                  // State A: Unpopulated Dropzone Input Field Frame
                  <div
                  ...
                  >
                    ...
                    <p className="text-sm font-semibold text-neutral-200">
                      Drag & drop movie clip or screenshot
                    </p>
                    <p className="text-xs text-neutral-500 mt-2">
                      Supports PNG, JPG, MP4, or MOV file signatures
                    </p>
                  </div>
                ) : (
                  // State B: Populated Preview Canvas Media Frame
                  <div className="w-full space-y-4">
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        disabled={analyzing}
                        className="flex-1 py-3.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-xs font-bold transition disabled:opacity-40"
                      >
                        Discard File
                      </button>
                      <button
                        ...
                        disabled={analyzing}
                        className="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 font-bold text-xs shadow-lg hover:opacity-95 transition disabled:opacity-40 flex items-center justify-center gap-2"
                      >
                        {analyzing ? <FaSync className="animate-spin text-xs" /> : <FaSparkles className="text-xs" />}
              ...
              {/* Status Loading Track Console Drawer Footer */}
              {analyzing && (
            ...
            {/* RIGHT COLUMN: Results Delivery Node Card Display */}
              ...
              <div className="bg-[#0b0b14]/60 border border-white/[0.06] rounded-[32px] p-6 backdrop-blur-xl shadow-2xl min-h-[380px] flex flex-col justify-between">
                <div>
                  ...
                  {!matchedMovie ? (
                    <div className="text-center py-24 text-xs font-mono text-white/20 tracking-widest uppercase leading-relaxed">
                      {!analyzing 
                        ? "Upload a scene screenshot clip to trigger discovery algorithms." 
                        : "Processing frame contours across databases..."}
                    </div>
                  ) : (
                    // State B: Match Found Layout Card Component Render Target
                ...
                {matchedMovie && (
                  <button
                    ...
                    className="mt-6 w-full py-3.5 rounded-xl bg-white/5 border border-white/10 text-xs font-bold text-neutral-200 hover:bg-white/10 hover:text-white transition flex items-center justify-center gap-2"
                  >
