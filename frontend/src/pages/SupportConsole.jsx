...
export default function SupportConsole() {
  ...
  const handleSubmitFeedback = async (e) => {
    ...
    try {
      ...
      setMessage("");
      navigate(-1); // Automatically send them back to where they came from
    } catch (err) {
      console.error("Developer contact pipeline crash:", err);
      alert("❌ Submission Failure: Permissions layer blocked writing to database.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#04040a] text-white relative overflow-hidden font-sans antialiased flex flex-col justify-between">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,#4c1d95,transparent_40%),radial-gradient(circle_at_bottom_left,#7f1d1d,transparent_40%)] opacity-50 pointer-events-none z-0" />
      ...
      <div className="relative z-10 flex items-center justify-between px-8 py-5 border-b border-white/[0.06] backdrop-blur-2xl bg-black/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black bg-gradient-to-r from-red-500 via-pink-500 to-purple-500 bg-clip-text text-transparent tracking-tight">
              ...
              <div className="flex flex-wrap gap-2">
                {quickComments[category].map((item, index) => (
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-pink-500/30 hover:bg-pink-500/5 text-xs text-white/70 hover:text-white transition-all duration-200 cursor-pointer active:scale-95 disabled:opacity-30"
                  >
                    {item.label}
                  </button>
                ))}
